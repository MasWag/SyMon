  ...
  void parse(const std::string &content) {
    ...
    for (uint32_t i = 0; i < rootSize; i++) {
      ...
      if (ts_node_type(child) == std::string("variables")) {
        ...
        auto id = std::string(content.begin() + ts_node_start_byte(idNode), content.begin() + ts_node_end_byte(idNode));
        p++;
...
public:
  ...
  static std::vector<::TimingConstraint<Time>> extractUpperBound(const std::vector<::TimingConstraint<Time>> &guard) {
    std::vector<::TimingConstraint<Time>> upperBound;
    ...
    for (const auto &constraint: guard) {
      ...
      if (constraint.odr == ::TimingConstraint<Time>::Order::lt || constraint.odr == ::TimingConstraint<Time>::Order::le) {
  ...
  static ParametricTimingConstraint extractUpperBound(const ParametricTimingConstraint &guard,
                                                      const VariableID clockIndex = 0) {
...
private:
    ...
    for (const auto &state: automaton.states) {
      for (const auto &[_, transitions]: state->next) {
        for (const auto &transition: transitions) {
          if (std::find(transition.resetVars.begin(), transition.resetVars.end(), lastClockId) !=
  ...
  static std::string parseDeclaration(const std::string &content, const TSNode &declarationNode) {
    if (ts_node_type(declarationNode) != std::string("string_definition") &&
        ts_node_type(declarationNode) != std::string("number_definition") &&
        ts_node_type(declarationNode) != std::string("parameter_definition")) {
    ...
    for (uint32_t i = 0; i < nodeSize; i++) {
      if (const TSNode child = ts_node_child(declarationNode, i); ts_node_type(child) == std::string("identifier")) {
        return std::string(content.begin() + ts_node_start_byte(child), content.begin() + ts_node_end_byte(child));
      }
    }
    throw std::runtime_error(
        makeErrorMessage("Declaration node does not contain identifier", content, declarationNode));
  }
  ...
  static RawSignature makeSignature(const std::string &content, const TSNode &signatureNode) {
    ...
    for (uint32_t i = 0; i < nodeSize; i++) {
      ...
      if (childType == "identifier") {
        name = std::string(content.begin() + ts_node_start_byte(child), content.begin() + ts_node_end_byte(child));
        ...
        throw std::runtime_error(makeErrorMessage(
            ("Expected identifier, string_definition, or number_definition node in signature but got: " + childType)
                .c_str(),
            content, child));
      }
    }
      ...
      if (it == this->globalStringVariables.end()) {
        if (this->localStringVariables) {
          ...
          if (it != this->localStringVariables->end()) {
            atomic = "x" + std::to_string(this->globalStringVariables.size() +
                                          std::distance(this->localStringVariables->cbegin(), it));
            return;
          }
        }
      ...
      if (it == this->globalNumberVariables.end()) {
        if (this->localNumberVariables) {
          ...
          if (it != this->localNumberVariables->end()) {
            atomic = "x" + std::to_string(this->globalNumberVariables.size() +
                                          std::distance(this->localNumberVariables->cbegin(), it));
            return;
          }
        }
  ...
  std::string parseNumericExpr(const std::string &content, const TSNode &constraintNode) const {
    ...
    throw std::runtime_error(makeErrorMessage(
        ("Expected numeric expression to have 0 or 3 children, but got: " + std::to_string(nodeSize)).c_str(), content,
        constraintNode));
  }
  ...
  StringConstraint parseStringConstraint(const std::string &content, const TSNode &constraintNode) const {
    ...
    if (ts_node_child_count(constraintNode) != 3) {
      throw std::runtime_error(
          makeErrorMessage("Expected string_constraint to have exactly 3 children", content, constraintNode));
    }
    const TSNode lhsNode = ts_node_child(constraintNode, 0);
    const TSNode opNode = ts_node_child(constraintNode, 1);
    const TSNode rhsNode = ts_node_child(constraintNode, 2);
  ...
  NumberConstraint parseNumericConstraint(const std::string &content, const TSNode &constraintNode) const {
    ...
    if (ts_node_child_count(constraintNode) != 3) {
      std::string innerContent = std::string(content.begin() + ts_node_start_byte(constraintNode),
                                             content.begin() + ts_node_end_byte(constraintNode));
      throw std::runtime_error(
          makeErrorMessage("Expected numeric_constraint to have exactly 3 children", content, constraintNode));
    }
    const TSNode lhsNode = ts_node_child(constraintNode, 0);
    const TSNode opNode = ts_node_child(constraintNode, 1);
    const TSNode rhsNode = ts_node_child(constraintNode, 2);
  ...
  void parseConstraintList(const std::string &content, TSNode parent, std::vector<StringConstraint> &stringConstraints,
  ...
  void parseInits(const std::string &content, const TSNode &initialConstraintsNode) {
    ...
    if constexpr (!std::is_same_v<TimingConstraint, ParametricTimingConstraint>) {
      throw std::runtime_error(
          makeErrorMessage("Giving initial constraints is only supported for parametric timing constraints", content,
                           initialConstraintsNode));
    }
    ...
    this->parseConstraintList(content, parent.value(), this->initialStringConstraints, this->initialNumberConstraints);
  }
  ...
  TimingConstraint parseTimingConstraint(const std::string &content, const TSNode &constraintNode,
    ...
    if (ts_node_type(parent) == std::string("intervals")) {
      ...
      if constexpr (std::is_same_v<TimingConstraint, ParametricTimingConstraint>) {
        const std::string lowerGuardString =
            "x" + std::to_string(clockIndex) + " " + (isLowerInclusive ? ">=" : ">") + " " + lowerBound;
        const std::string upperGuardString =
            "x" + std::to_string(clockIndex) + " " + (isUpperInclusive ? "<=" : "<") + " " + upperBound;
        try {
          auto lowerGuard = boost::lexical_cast<ParametricTimingConstraintHelper>(lowerGuardString);
          auto upperGuard = boost::lexical_cast<ParametricTimingConstraintHelper>(upperGuardString);
        ...
        return {boost::lexical_cast<::TimingConstraint<Time>>("x" + std::to_string(clockIndex) + " " +
                                                        (isLowerInclusive ? ">=" : ">") + " " + lowerBound),
                boost::lexical_cast<::TimingConstraint<Time>>("x" + std::to_string(clockIndex) + " " +
    ...
    if (ts_node_type(parent) == std::string("half_guard")) {
      ...
      if (comparator != "<" && comparator != "<=" && comparator != ">" && comparator != ">=" && comparator != "=" &&
          comparator != "<>") {
        throw std::runtime_error(makeErrorMessage(
            ("Expected comparator to be '<', '<=', '>', '>=', '=', or '<>', but got: " + comparator).c_str(), content,
            comparatorNode));
      }
      if (comparator == "=") {
        comparator = "==";
      ...
      if constexpr (std::is_same_v<TimingConstraint, ParametricTimingConstraint>) {
        auto guard = boost::lexical_cast<ParametricTimingConstraintHelper>("x" + std::to_string(clockIndex) + " " +
                                                                           comparator + " " + expr);
        ...
        return {
            boost::lexical_cast<::TimingConstraint<Time>>("x" + std::to_string(clockIndex) + " " + comparator + " " + expr)};
      }
    }
    throw std::runtime_error(makeErrorMessage(
        ("Expected half_guard node or intervals node, but got: " + std::string(ts_node_type(parent))).c_str(), content,
        parent));
  }
  ...
  std::vector<Action> parseActionList(const std::string &content, const TSNode &identifierListNode) const {
    ...
    for (std::size_t i = 0; i < size; ++i) {
      ...
      auto identifier = std::string(content.begin() + ts_node_start_byte(identityNode),
                                    content.begin() + ts_node_end_byte(identityNode));
      auto it = std::find_if(this->signatures.begin(), this->signatures.end(),
                             [&identifier](const RawSignature &sig) { return sig.name == identifier; });
      if (it == this->signatures.end()) {
        throw std::runtime_error(
            makeErrorMessage(("Undeclared action: " + identifier).c_str(), content, identifierListNode));
      }
  ...
  Automaton parseExpr(const std::string &content, const TSNode &exprNode) {
    if (ts_node_type(exprNode) != std::string("expr")) {
      throw std::runtime_error(makeErrorMessage(
          (std::string("Expected expr node but found ") + ts_node_type(exprNode)).c_str(), content, exprNode));
    }
    if (const uint32_t nodeSize = ts_node_child_count(exprNode); nodeSize != 1) {
      throw std::runtime_error(makeErrorMessage(
          ("Expected expr node to have exactly one child, but got: " + std::to_string(nodeSize)).c_str(), content,
          exprNode));
    }
    ...
    if (kind == "identifier") {
      const auto identifier =
          std::string(content.begin() + ts_node_start_byte(child), content.begin() + ts_node_end_byte(child));
      ...
      if (ts_node_type(identifierNode) != std::string("identifier")) {
        throw std::runtime_error(
            makeErrorMessage("Expected atomic node to have identifier child", content, identifierNode));
      }
      ...
      if (!isUnobservable) {
        ...
        if (!signature) {
          throw std::runtime_error(
              makeErrorMessage(("Undeclared signature: " + signatureName).c_str(), content, identifierNode));
        }
      ...
      if (argNode) {
        ...
        for (std::size_t i = 0; i < ts_node_child_count(*argNode); ++i) {
          ...
          if (ts_node_type(cur) == std::string("identifier")) {
            auto identifier =
                std::string(content.begin() + ts_node_start_byte(cur), content.begin() + ts_node_end_byte(cur));
      ...
      if (std::optional<TSNode> guardNode = ts_node_child_by_type(child, "guard_block"); guardNode.has_value()) {
        ...
        if (constraintNode.has_value()) {
          this->parseConstraintList(content, constraintNode.value(), transition.stringConstraints,
        ...
        if (updateNode.has_value()) {
          ...
          for (std::size_t i = 0; i < updateSize; ++i) {
            ...
            if (ts_node_type(identifierNode) != std::string("identifier")) {
              throw std::runtime_error(
                  makeErrorMessage("Expected assignment node to have identifier child", content, identifierNode));
            }
            auto identifier = std::string(content.begin() + ts_node_start_byte(identifierNode),
                                          content.begin() + ts_node_end_byte(identifierNode));
            // Check if the identifier is a string or number variable. Here, the assigned value must be
            // global.
            if (auto it = std::find(this->globalStringVariables.begin(), this->globalStringVariables.end(), identifier);
                it != this->globalStringVariables.end()) {
              ...
              transition.update.stringUpdate.push_back(
                  boost::lexical_cast<std::remove_reference_t<decltype(transition.update.stringUpdate.front())>>(
                      asString));
            } else if (auto it = std::find(this->globalNumberVariables.begin(), this->globalNumberVariables.end(),
                                           identifier);
                       it != this->globalNumberVariables.end()) {
              ...
              std::string asString = "x" + std::to_string(id) + " := " + value;
              transition.update.numberUpdate.push_back(
                  boost::lexical_cast<std::remove_reference_t<decltype(transition.update.numberUpdate.front())>>(
                      asString));
            } else {
              throw std::runtime_error(makeErrorMessage(("Undeclared variable in assignment: " + identifier).c_str(),
                                                        content, identifierNode));
            }
          }
        }
      }
      ...
      return concatenate<StringConstraint, NumberConstraint, TimingConstraint, Update, Time>(std::move(lhs), std::move(rhs));
      ...
      while (p < ts_node_child_count(child)) {
        ...
        if (nType == "}" || nType == "and" || nType == "{") {
          ...
          throw std::runtime_error(
              makeErrorMessage((std::string("Expected expr in all_of but got ") + nType).c_str(), content, child));
        }
      ...
      while (p < ts_node_child_count(child)) {
        ...
        if (nType == "}" || nType == "or" || nType == "{") {
          ...
          throw std::runtime_error(
              makeErrorMessage((std::string("Expected expr in one_of but got ") + nType).c_str(), content, child));
        }
        Automaton e = this->parseExpr(content, n);
        if (first) {
          acc = std::move(e);
      ...
      if (ts_node_is_null(innerNode) || ts_node_is_null(intervalNode)) {
        throw std::runtime_error(
            makeErrorMessage("Expected 'expr' and 'timing_constraint' in within or time_restriction", content, child));
      }
      ...
      Automaton result = timeRestriction<StringConstraint, NumberConstraint, TimingConstraint, Update, Time>(std::move(innerExpr), guard);
        ...
        assert(std::all_of(result.states.begin(), result.states.end(), [&](const auto &state) {
          return std::all_of(state->next.begin(), state->next.end(), [&](const auto &transitionsPair) {
            ...
            return std::all_of(transitions.begin(), transitions.end(), [&](const auto &transition) {
              return transition.guard.space_dimension() == this->parameters.size() + result.clockVariableSize;
            });
          });
        }));
      }
      ...
      if constexpr (std::is_same_v<TimingConstraint, std::vector<::TimingConstraint<Time>>>) {
        ...
        if (!upperBound.empty()) {
          ...
          addConstraintToAllTransitions<StringConstraint, NumberConstraint, TimingConstraint, Update, Time>(result, upperBound);
        }
