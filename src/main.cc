...
template<typename TAType, typename BoostTAType, typename Number, typename Timestamp, typename Monitor, typename Printer,
    typename StringConstraint, typename NumberConstraint, typename TimingConstraintType, typename UpdateType>
int execute(const std::string &timedAutomatonFileName,
            const std::string &signatureFileName,
            const std::string &timedWordFileName,
            bool useNewSyntax = false) {
    TAType TA;
    Signature signature;
...
int main(int argc, char *argv[]) {
    ...
    visible.add_options()
            ...
            ("new,n", "use the experimental syntax of SyMon")
            ("version,V", "version")
    ...
    if ((vm.count("boolean") && vm.count("dataparametric") && vm.count("parametric")) ||
        (vm.count("boolean") + vm.count("dataparametric") + vm.count("parametric") > 1)) {
    ...
    if (vm.count("new")) {
        ...
        if (vm.count("parametric")) {
            ...
            return execute<ParametricTA, BoostPTA, Coefficient, Coefficient, ParametricMonitor, ParametricPrinter,
            ...
            return execute<DataParametricTA, DataParametricBoostTA, Coefficient, double, DataParametricMonitor,
                DataParametricPrinter,
                Symbolic::StringConstraint, Symbolic::NumberConstraint, std::vector<TimingConstraint>,
                Symbolic::Update>(
                timedAutomatonFileName, signatureFileName, timedWordFileName, true);
        } else {
        ...
        return execute<DataParametricTA, DataParametricBoostTA, Coefficient, double, DataParametricMonitor,
            DataParametricPrinter,
            Symbolic::StringConstraint, Symbolic::NumberConstraint, std::vector<TimingConstraint>, Symbolic::Update>(
            timedAutomatonFileName, signatureFileName, timedWordFileName, false);
    } else {
        // boolean
        return execute<NonParametricTA<Number>, NonParametricBoostTA<Number>, Number, double, BooleanMonitor<Number>,
