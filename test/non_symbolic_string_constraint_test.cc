...
BOOST_AUTO_TEST_SUITE(NonSymbolicStringConstraintTests)
  BOOST_AUTO_TEST_SUITE(LexicalCastTests)
    using boost::lexical_cast;

    BOOST_AUTO_TEST_CASE(variable) {
      std::string str = "x12";
      using type = NonSymbolic::StringAtom;
    ...
    BOOST_AUTO_TEST_CASE(varEqConstant) {
      std::string str = "x0 == 'y'";
      using type = NonSymbolic::StringConstraint;
      type result = lexical_cast<type>(str);
      BOOST_CHECK_EQUAL(result.kind, NonSymbolic::StringConstraint::kind_t::EQ);
    ...
    BOOST_AUTO_TEST_CASE(varEqConstants) {
      std::string str = "{x0 == 'y'}";
      using type = std::vector<NonSymbolic::StringConstraint>;
      type result = lexical_cast<type>(str);
