...
namespace NonSymbolic {
  ...
  bool eval(const std::vector<StringConstraint> &stringConstraints, StringValuation &stringEnv,
            const std::vector<NumberConstraint < Number>>

  &numConstraints,
  const NumberValuation <Number> &numEnv
  ) {
  return
  std::all_of(stringConstraints
  .

  begin(), stringConstraints
...
std::all_of(numConstraints
.

begin(), numConstraints
...
[&numEnv](
const NumberConstraint <Number> &constraint
) {
return constraint.
eval(numEnv);
});
